	...
	{{-- @if(Auth::user())
		@if(Auth::user()->hasPerfil('Admin'))
			<script  type="text/javascript">
				Echo.private( '{{env('APP_NAME') .'.login' }}'  )
				.listen('LoginEvent', (e) => {
					//console.log(e);
					toastSucesso(e.user.nome , ' Logou no sistema', function() {} , false ); 
				}); 
			</script> 
		@endif
	@endif --}}
...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-125557609-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-125557609-1');
</script>
