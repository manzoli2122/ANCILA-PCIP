			...
			<div class="container-fluid">   
				<crudCard>
					...
					<div class="card-footer text-right">
						<crudBotaoVoltar url="/usuario" />   
						<router-link :to="'/usuario/' + this.$route.params.id + '/perfil/historico'" exact  class="btn btn-warning">
							<i class="fa fa-database"></i> Historico
						</router-link>
					</div>
				</crudCard>  

				<formAdicionarPerfil v-if="perfis.length > 0" v-on:perfilAdicionado="perfilAdicionado($event)" :perfis="perfis" :url="url + this.$apiUsuario"> </formAdicionarPerfil>    
	...
	export default {
		...
		data() {
			return {    
				...
				config: {
					exclusao:{
						url:this.url + this.$apiUsuario + '/' + this.$route.params.id + '/delete/perfil'  ,
						evento:'perfilRemovido',
						item:'Perfil',
					},
					order: [[ 1, "asc" ]],
					ajax: { 
						url: this.url + this.$apiUsuario + '/' + this.$route.params.id + '/perfil/datatable'
					},
					columns: [
		...
		methods: {
			...
			buscarPerfis() {
				alertProcessando();
				axios.get(this.url + this.$apiUsuario + "/" + this.$route.params.id + "/perfil/adicionar")
				.then(response => {
